...
| --- | --- | --- | --- |
| Raw rounded scoreline outcome accuracy | Accuracy of the winner/draw implied by rounded goal predictions before reconciliation. | Higher is better | Checks whether the goal model points to the right match result, but it is a diagnostic rather than the final submission metric. |
...
| `dashboard_team_profiles.csv` | One row per team | 48 unique teams, ranking, Elo, adjusted-form, squad, event, player star-power, and completeness fields present. |
| `dashboard_player_power_profiles.csv` | One row per squad player | Roster players include team, position, caps, international goals, top-league form, market-value context, and player star-power proxy. |
